	...
	<body>
		...
		<div id="show-musee" class="content scaffold-show" role="main">
			...
			<ol class="property-list musee">
				...
				<g:if test="${museeInstance?.demandesVisite}">
				<li class="fieldcontain">
					<span id="demandesVisite-label" class="property-label"><g:message code="musee.demandesVisite.label" default="Demandes Visite" /></span>
					
						<g:each in="${museeInstance.demandesVisite}" var="d">
						<span class="property-value" aria-labelledby="demandesVisite-label"><g:link controller="demandeVisiteMusee" action="show" id="${d.id}">${d?.encodeAsHTML()}</g:link></span>
						</g:each>
